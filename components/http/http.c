...
void http_gett(struct url_data *data) {
    ...
    sprintf(urlendpoint, "/api/v1/%s", data->token);
    ...
    if (err == ESP_OK) {
        ESP_LOGI(TAG, "HTTP GET Status = %d, content_length = %"PRId64, esp_http_client_get_status_code(client), esp_http_client_get_content_length(client));
    } else {
    ...
    for (int i=0; i<strlen(local_response_buffer); i++) {
        data->resp[i] = local_response_buffer[i];
    }
...
void http_get(struct url_data * data) {
    ...
    http_get_task(NULL);
#else
    xTaskCreate(&http_get_task, "http_test_task", 8192, data, 5, NULL);
#endif
}
